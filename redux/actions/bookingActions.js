...
export const checkBooking =
  (roomId, checkInDate, chekcOutDate) => async (dispatch) => {
    try {
      dispatch({ type: CHECK_BOOKING_REQUEST });
      let link = `/api/bookings/check?roomId=${roomId}&checkInDate=${checkInDate}&checkOutDate=${chekcOutDate}`;
...
export const myBookings = (authCookie, req) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(`${origin}/api/bookings/me`, config); //ami ei config object na pathayle ki hoto eita dekha dorkar .
